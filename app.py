...
from client import wiz_run,Client
import asyncio
import pywizlight
...
def make_window(theme):
    ...
    input_layout = [
        [sg.Text("IP RANGE")],
        [sg.Input(key='-IPRANGE-')],
        [sg.Button('FIND',key='-FIND-')],
         [sg.Listbox(values=[],select_mode="LISTBOX_SELECT_MODE_MULTIPLE",key='-LIST-',size=(20,12))],
        [sg.Button('TURN OFF',key='-TURNOFF-')],
        [sg.Button('TURN ON',key='-TURNON-')],
         #[sg.Button('SHOW FEATURES',key="-FEATURES-")],
         #[sg.In(key='-CHOICE-'),sg.B('Color Picker')],
        # [sg.Ok(),sg.Cancel()],
        [sg.Text(
            'BRIGHTNESS',size=(10,1)
        )],
         [sg.Slider(range=(0,255),orientation='h',key='-BRIGHTNESS-')],
         #[sg.Listbox(values=[1,2,3,4],select_mode="LISTBOX_SELECT_MODE_SINGLE",key='-SCENE-',size=(20,12))],
         [sg.Button('Update',key='-UPDATE-')] ]
    layout = [ [sg.Menubar(menu_def)]]
    layout += [[sg.TabGroup([[
        sg.Tab('Settings',input_layout)
    ]])]]
    window=sg.Window('WIZ CLIENT',layout,grab_anywhere=True,resizable=True,margins=(0,0),finalize=True,keep_on_top=True)
...
async def main():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Cancel':
            ...
            print(ip_range)
            client = await Client.create(ip_range=ip_range)
            devices = client.list_devices() 
            window['-LIST-'].update(values=devices)
            continue
        elif event == '-TURNON-':
            ip = window['-LIST-'].get()[0]
            print(ip)
            ...
            ip = values['-LIST-'][0]
            name,kelvin_range,color_tmp_supported,brightness_supported,color_supported,effect= await client.get_features(ip)

            print(name,kelvin_range,color_tmp_supported,brightness_supported,color_supported,effect)
            continue
        elif event == '-UPDATE-': 
            brightness = values['-BRIGHTNESS-']
            print(brightness)
            ip = values['-LIST-'][0]
            if await client.change_brightness(brightness,ip) == False:
                print("error occured")
                window.close()
            ''' 
            try:
                ...
                ip = window['-LIST-'].get()
                scene = window['-SCENE-'].get()
                print(ip)
                print(scene)
                await client.change_scene(ip=ip[0],scene=scene[0])
            ...
            window.un_hide()
            color_chosen_rgb = color_chosen.lstrip("#")
            color_chosen_rgb = tuple(int(color_chosen_rgb[i:i+2],16) for i in (0,2,4))
            ip = values['-LIST-'][0]
            await client.change_color(color=color_chosen_rgb,ip=ip)
    window.close()
